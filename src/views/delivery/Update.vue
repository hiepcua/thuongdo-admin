  ...
  <div class="delivery-add" id="ST2022">
    ...
    <div class="frame-content-common-2022">
      <div class="frame-content-title-2022-v2">
        ...
        <span class="title">Đơn hàng - Cập nhật giao hàng</span>
      </div>
                ...
                <div class="delivery-add-info">
                  ...
                  >
                    <CreateDeliveryGarage
                      ...
                      :transporter_detail_id="modelData.transporter_detail_id"
...
export default {
  ...
  data() {
    return {
      ...
      // Thêm, Xóa các kiện khỏi danh sách
      packageTempAdd: [],
      packageTempRemove: [],
    };
  },
  ...
  methods: {
    ...
    fetchModel () {
      axios.get("delivery/" + this.$route.params.delivery_id).then((response) => {
        switch (response.code) {
          case 200:
            this.modelData.transporter_id = response.data.transporter_id;
            this.modelData.transporter_detail_id = response.data.transporter_detail_id;
            this.modelData.type = response.data.type;
            this.modelData.payment = response.data.payment;
            this.modelData.customer_delivery_id = response.data.customer_delivery_id;
            this.modelData.note = response.data.note;
            this.modelData.date = response.data.date;

            this.modelData.customer_id = response.data.customer.id;
            this.modelData.customer_name = response.data.customer.name;
            this.modelData.customer_phone = response.data.customer.phone_number;

            this.listPack = response.data.packages;

            this.listPack.map(function (i) {
              let pack_status = {
                value: "status_10",
                name: "Đến kho VN",
                color: "#022652",
              ...
              i.pack_status = pack_status;
              i.pack_id = i.id;
              return i;
            });

            this.listPack = JSON.parse(JSON.stringify(this.listPack));
      ...
      axios.get("common/delivery", { params: data }).then((response) => {
        switch (response.code) {
          case 200:
            ...
            this.listCategory.transporters.forEach((item) => {
              if (item.id == this.modelData.transporter_id) {
    ...
    deletePackTap1(item) {
      ...
      if (index > -1) {
        ...
        let itemRemove = this.listPack[index];
        this.packageTempRemove.push(itemRemove.id);
    ...
    submitForm() {
      ...
      data.packages = {
        add: Array.from(new Set(this.packageTempAdd)),
        remove: Array.from(new Set(this.packageTempRemove)),
      };
      ...
      axios.put("delivery/" + this.$route.params.delivery_id, data).then((response) => {
        this.canSubmit = true;
        switch (response.code) {
          case 200:
            this.$notify({
      ...
      this.itemAddPackArr.forEach((item) => {
        if (item.checked) {
          ...
          if (canPush) {
            ...
            this.packageTempAdd.push(temp.pack_id);
          }
        }
      });
