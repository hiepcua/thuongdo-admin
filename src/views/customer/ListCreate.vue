<template>
  <div class="customer-common customer-list-create frame-content-common-2022">
    ...
    <div class="tab-navigation">
      <span class="item" :class="[tab == 'info' ? 'active' : '']"
        >Thông tin tài khoản</span
      >
      <span class="item" :class="[tab == 'address' ? 'active' : '']">
        Địa chỉ nhận hàng
      </span>
      <!--      <span-->
      <!--        class="item"-->
      <!--        :class="[tab == 'bank' ? 'active' : '']"-->
      <!--        @click="changeTap('bank')"-->
      <!--        >Ngân hàng</span-->
      <!--      >-->
      <!--      <span-->
      <!--        class="item"-->
      ...
      <!--      <span-->
      <!--        class="item"-->
      <!--        :class="[tab == 'partner' ? 'active' : '']"-->
      <!--        @click="changeTap('partner')"-->
      <!--        >Đối tác</span-->
      <!--      >-->
      <!--      <span-->
      <!--        class="item"-->
      <!--        :class="[tab == 'history' ? 'active' : '']"-->
      <!--        @click="changeTap('history')"-->
    ...
    >
        ...
        <el-row :gutter="30">
          ...
          <el-col :span="12">
            <el-form-item
              label="Ngày sinh"
              prop="bod"
              class="item-form-normal padding-input-bod"
            >
              <el-date-picker
      ...
      >
        <div
          v-for="(customerDelivery, index) in listAdd"
          :key="customerDelivery.time_created"
    ...
    <div class="dialog-frame-normal-2022">
      <el-dialog :visible.sync="dialogAddAddressVisible" width="700px" center>
        <span
          slot="title"
        ...
        <template #footer>
          <div class="d-flex flex-row justify-content-end align-items-end mr-1">
            <div
              class="d-flex frame-btn-cancel-customer-td-2022 justify-content-center align-items-center mr-1"
...
export default {
  ...
  methods: {
    ...
    deleteAddress(index) {
      this.listAdd = this.listAdd.filter((_, i) => i !== index);
    },
    getCategory() {
      ...
      this.$refs["address-customer-form"].validate((valid) => {
        if (valid) {
          if (this.adding) {
            ...
            item["time_created"] = moment().unix();
            this.listAdd.push(item);
          } else {
            let index = this.listAdd.findIndex(
              (e) => e.time_created === this.addressForm.time_created
            );
            this.listAdd[index] = Object.assign(this.addressForm);
          }
          this.addressForm = {
            receiver: "",
            phone_number: "",
    ...
    changeReceiver(row) {
      for (let i = 0; i < this.listAdd.length; i++) {
        if (row.time_created === this.listAdd[i].time_created) {
          this.listAdd[i].is_default = 1;
        } else {
          this.listAdd[i].is_default = 0;
        }
      }
    },
    submitForm() {
      this.$refs["customer-create-form"].validate((valid) => {
        if (valid) {
          this.customerForm.bod
            ...
            : "";
          let adds = [];
          this.listAdd.forEach((e) => {
            delete e.time_created;
            adds.push(e);
          });
          this.customerForm.customer_delivery = adds;
          axios.post("/customer", this.customerForm).then((response) => {
            switch (response.code) {
              case 200:
                this.$notify({
