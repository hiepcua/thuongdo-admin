...
export default {
  ...
  methods: {
    // detail
    fetchFineDetai(loading = true) {
      if (loading) {
        this.loading = true;
      }
      axios.get("fine/" + this.$route.params.fine_id).then((response) => {
        this.loading = false;
        switch (response.code) {
          case 200:
            this.listDetail = response.data;
            this.listCreateFine.order_code = response.data.info.order_code;
            this.listCreateFine.bill_code = response.data.info.bill_code;
            this.listCreateFine.amount = response.data.amount;
            this.listCreateFine.reason = response.data.reason;
            this.listCreateFine.solution = response.data.solution;
            this.listCreateFine.user_id = response.data.user.id;
            this.listCreateFine.type = response.data.type.value;
            break;
          case 500:
            this.$notify({
              title: "Có lỗi xảy ra",
    ...
    checkDataPushError() {
      ...
      // if (this.listCreateFine.order_code == "") {
      //   this.listCreateFine.order_code_err = true;
      //   flag = true;
      // }
      // if (this.listCreateFine.bill_code == "") {
      //   this.listCreateFine.bill_code_err = true;
      //   flag = true;
      // }
    ...
    submitForm() {
      ...
      axios.put("fine/" + this.$route.params.fine_id, data).then((response) => {
        this.canSubmit = true;
        switch (response.code) {
          case 200:
            this.$notify({
              title: "Thành công",
              message: "Sửa nộp phạt thành công",
              type: "success",
            });
            this.$router.push({
              name: "fines-lists",
