  ...
  <div class="fund-transaction-list" id="ST2022">
    <div class="frame-content-common-2022">
      <div class="frame-content-title-2022-v2">
        ...
        <span class="title" @click="initDefault()">Lịch sử thu chi tiền Việt Nam</span>
      </div>
      ...
      <div class="table-element-custom-2022-v2 fund-transaction-list-table mt-3">
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <div class="item-action" v-if="scope.row.fund_type_pay_code == 1 && scope.row.lock == 0">
              ...
              <div class="item-action">
                <el-tooltip class="item" effect="dark" content="Lịch sử" placement="top" >
                  <img src="@/static/icon/fund-transaction/list/ic_round-history.svg" @click="showHistory(scope.row)" />
                </el-tooltip>
              </div>
            </template>
          </el-table-column>
            ...
            <div class="form-item">
              ...
              <div class="form-input">
                <number
                  v-bind="number"
                  placeholder="Số tiền"
                  v-model.lazy="modelAdd.money"
                  class="frame-input-common-2022 custom-inpupt-formart-2023"
                >
                </number>
              </div>
            </div>
      ...
      <div class="frame-dialog-no-close-1988 fund-transaction-add">
        <el-dialog
          ...
          width="70%"
          :close-on-click-modal="false"
          :visible.sync="dialogShowHistory"
          :show-close="false"
        >
          <div class="header-1988 header-1988-blue">
            <img src="@/static/icon/fund-transaction/list/dola-popup.svg" />
            <span>Lịch sử sửa đổi thu chi tiền Việt Nam</span>
          </div>
          <div class="body-1988">
            <div class="table-element-custom-2022-v2">
              ...
              >
                ...
                <el-table-column label="Người thay đổi" width="200">
                  <template slot-scope="scope">
                    <span>{{ scope.row.user_request_name }}</span>
                  </template>
                </el-table-column>
                <el-table-column label="Thông tin">
                  <template slot-scope="scope">
                    <span>{{ scope.row.action }} <span v-if="scope.row.column">:</span> {{ scope.row.column }}</span>
                  </template>
                </el-table-column>
                <el-table-column prop="value_old" label="Giá trị cũ" width="160">
                  <template slot-scope="scope">
                    <span v-if="scope.row.value_old">
                      <span v-if="isNumeric(scope.row.value_old)">{{ scope.row.value_old | vnd }}</span>
                      <span v-else>{{ scope.row.value_old }}</span>
                    </span>
                  </template>
                </el-table-column>
                <el-table-column prop="value_new" label="Giá trị mới" width="160">
                  <template slot-scope="scope">
                    <span v-if="scope.row.value_new">
                      <span v-if="isNumeric(scope.row.value_new)">{{ scope.row.value_new | vnd }}</span>
                      <span v-else>{{ scope.row.value_new }}</span>
                    </span>
                  </template>
                </el-table-column>
              </el-table>
...
export default {
  ...
  methods: {
    ...
    submitUpdateFundTransaction() {
      ...
      this.modelAdd.money_update = this.modelAdd.money;
    ...
    // Xem lịch sử
    showHistory(item) {
      this.dialogShowHistory = true;
      this.arrHistory = item.logs;
    },
