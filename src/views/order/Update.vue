    ...
    <div class="frame-content-common-2022">
            ...
            <div class="order-update-warehouse">
              <div class="order-update-warehouse-code" v-if="orderDetail.info">
                ...
                <div class="line-item">
                  <el-select
                    :style="{ backgroundColor: orderDetail.info.status.color }"
                    v-model="orderDetail.info.status.value"
                    placeholder="chọn trạng thái"
                    class="frame-input-common-2022 status-withdrawal-2022"
                    popper-class="frame-select-popper-customer-2022"
                    @change="updateStatus()"
                    :disabled="orderDetail.info.status.value == 'status_4' || orderDetail.info.status.value == 'status_6'"
                  >
                    <el-option
          ...
          <div class="hub-1">
            ...
            <div class="hub-1-2">
              ...
              <div
                class="item-check"
                @click="
                  item.suppliers.services.is_shock_proof =
                    !item.suppliers.services.is_shock_proof
                "
              >
                <img
                  src="@/static/icon/order/edit/tic_3.svg"
                  v-if="item.suppliers.services.is_shock_proof"
                />
                <img src="@/static/icon/order/edit/tic_4.svg" v-else />
                <span>Chống sốc</span>
              </div>
      ...
      <div class="d-flex justify-content-end pt-3">
        ...
        <span
          ...
          class="btn-crud-2022 frame-hover-2022 btn-warning d-none"
          :class="!canSubmit ? 'disable' : ''"
          @click="quoteConfirm()"
        >
          <span v-if="!canSubmit">
...
export default {
  ...
  methods: {
    ...
    // sửa trạng thái
    updateStatus() {
      this.updateStatusActoin();
    },
    updateStatusActoin() {
      let data = {
        status: this.orderDetail.info.status.value,
      };
      axios
        .patch("order/status/" + this.orderDetail.info.id, data)
        .then((response) => {
          switch (response.code) {
            case 200:
              this.$notify({
      ...
      this.listNCC.forEach((item) => {
        let temp = {
          ...
          modifies: {
            ...
            is_shock_proof: item.suppliers.services.is_shock_proof,
    ...
    caculaterRealtime() {
      ...
      this.listNCC.forEach((item, index) => {
        ...
        if (item.accounting.china_shipping_cost) {
          sum_total_amount_line +=
            Number.parseFloat(item.accounting.china_shipping_cost) *
            this.listConfig.exchange_rate;
        }
    ...
    changePrice() {
      ...
      axios.patch("accounting/order-fee", data).then((response) => {
        switch (response.code) {
          ...
          default:
            this.$notify.error({
              title: "Error",
              message: "Lỗi lấy phí đặt hàng và chiết khấu",
            });
            break;
        }
      });
      ...
      axios.patch("accounting/inspection", data2).then((response) => {
        switch (response.code) {
          case 200:
            this.nccSelect.accounting.inspection_cost = response.data;
            break;
          default:
            this.$notify.error({
              title: "Error",
              message: "Lỗi lấy phí kiểm đếm.",
            });
            break;
        }
      });
