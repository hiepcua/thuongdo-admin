<template>
  <div class="home-index frame-content-common-2022 order-report index-report" id="ST2022">
    ...
    <div v-loading="loading" class="report-info">
      <div class="content-report-frame-2023" v-if="listData.arr_current">
        <div class="content-box-report">
          <span class="numberReport color2">{{ listData.arr_current.total_waiting_quote }}</span>
          <span class="reportInfo">Tổng đơn chờ báo giá</span>
        </div>
        <div class="content-box-report">
          <span class="numberReport color3">{{ listData.arr_current.total_waiting_deposit }}</span>
          <span class="reportInfo">Tổng đơn đã báo giá</span>
        </div>
        <div class="content-box-report">
          <span class="numberReport color5">{{ listData.arr_current.total_done }}</span>
    ...
    <div class="d-flex justify-content-center">
      <div class="chart-customer-column">
        ...
        <highcharts class="chart" :options="chartQuoteLine"></highcharts>
      </div>
      <div class="chart-customer-column ml-1">
        <span class="chart-title">Biểu đồ đặt hàng theo tháng</span>
        <highcharts class="chart" :options="chartQuoteLineMonth"></highcharts>
      </div>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    fetchChartQuote() {
      ...
      axios.get("statistic/slow", { params: filter }).then((response) => {
        switch (response.code) {
          case 200:
            this.listChartQuote = response.data;
            this.chartQuoteLine.series = response.data.chart_user_type_3.chart_day;
            this.chartQuoteLineMonth.series = response.data.chart_user_type_3.chart_month;
            break;
          default:
            this.$notify.error({
              title: "Error",
...
/* biểu đồ  */
.order-report.index-report .chart-customer-column {
  width: 50%;
  background: #fff;
  padding-top: 22px;
  margin: 10px;
